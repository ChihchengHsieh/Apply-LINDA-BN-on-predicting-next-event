...
class BPI2012Dataset_V2(Dataset):
    ...
    def __init__(self, file_path: str, preprocessed_folder_path: str, preprocessed_df_type: PreprocessedDfType, device: torch.device = torch.device("cpu"), include_types: list[ActivityType] = [ActivityType.A, ActivityType.W, ActivityType.O]) -> None:
        ...
        self.preprocessed_folder_path = os.path.join(
            preprocessed_folder_path, BPI2012Dataset_V2.get_type_folder_name(include_types))
    ...
    def get_file_name_from_preprocessed_df_type(preprocessed_df_type: PreprocessedDfType):
        if preprocessed_df_type == PreprocessedDfType.Pickle:
            return BPI2012Dataset_V2.pickle_df_file_name
        else:
    ...
    def preprocessed_data_exist(preprocessed_folder_path: str, preprocessed_df_type: PreprocessedDfType, ):
        file_name = BPI2012Dataset_V2.get_file_name_from_preprocessed_df_type(
            preprocessed_df_type)
        df_path = os.path.join(preprocessed_folder_path,  file_name)
        vocab_dict_path = os.path.join(
            preprocessed_folder_path, BPI2012Dataset_V2.vocab_dict_file_name)
    ...
    def save_preprocessed_data(self):
        ...
        ############ Store vocab_dict ############
        vocab_dict_path = os.path.join(
            self.preprocessed_folder_path, BPI2012Dataset_V2.vocab_dict_file_name)
